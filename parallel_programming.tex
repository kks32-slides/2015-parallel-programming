...
%******************************* Frame *****************************************
%*******************************************************************************
\begin{frame}[fragile]{OpenACC Kernels}

\begin{itemize}
\item The kernels construct expresses that a region may contain parallelism
and the compiler determines what can safely be parallelized.
\item The compiler identifies 3 parallel loops and generates 3 kernels.
\end{itemize}
\centering
\begin{minted}[mathescape, linenos,
               numbersep=5pt,
...
{
  for(int i=0; i<N; i++)
    b[i] = b[i] * b[i]; //kernel 1
  for(int i=0; i<N; i++)
    a(i) = b(i) + a(i); //kernel 2
  for(int i=0; i<N; i++)
    b(i) = b(i) / a(i); //kernel 3
}
\end{minted}
...
%******************************* Frame *****************************************
%*******************************************************************************
\begin{frame}{OpenACC parallel loop vs. kernels}
\begin{columns}
\column{0.49\linewidth}
\textbf{OpenACC Parallel Loop}
\begin{itemize}
\item Requires analysis by programmer to ensure safe parallelism
\item Straightforward path from OpenMP
\end{itemize}
\column{0.49\linewidth}
\textbf{OpenACC Kernel}
\begin{itemize}
\item Compiler performs parallel analysis and parallelises what it
believes safe
...
\end{columns}
\vskip3em
\textbf{\textit{Both approaches are equally valid and can perform equally well}}
\end{frame}
